...
def test_parser_read_magres_json():
    ...
    assert (archive.data.ccpnc_metadata.material_properties
            .chemical_name == "Kaolinite")
    assert (archive.data.ccpnc_metadata.publication_record.doi == 
            "10.1016/j.clay.2018.12.013")
    assert archive.data.ccpnc_metadata.ccpnc_record.license == "pddl"
    assert (archive.data.ccpnc_metadata.external_database_reference
            .external_database_reference_code == "1896953")
...
def test_parser_read_magres_csv():
    ...
    # Check some key metadata fields
    assert (archive.data.ccpnc_metadata.material_properties
            .chemical_name.startswith("5-Hydroxy"))
    assert (archive.data.ccpnc_metadata.publication_record.doi == 
            "10.1002/anie.201908914")
    assert archive.data.ccpnc_metadata.ccpnc_record.license == "pddl"
    assert (archive.data.ccpnc_metadata.external_database_reference
            .external_database_reference_code == "BINMEQ05")
