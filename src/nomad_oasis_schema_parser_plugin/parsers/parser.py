...
class CCPNCMagresParser(MagresParser):
    ...
    def _process_magnetic_shielding_entry(
        ...
        if entity_ref is None or value is None:
            logger.warning(f"Skipping entry {i}: missing entity_ref or value.")
            return None
        atom = next((ps for ps in particle_states_ref if entity_ref is ps), None)
        if atom is None:
            logger.warning(
                f"Skipping entry {i}: could not match entity_ref to any "
                "particle_state."
            )
            return None
        chemical_symbol = getattr(atom, 'chemical_symbol', None)
        if chemical_symbol is None:
            logger.warning(f"Skipping entry {i}: could not resolve chemical_symbol.")
            return None
        try:
            tensor = NMRTensor(np.array(value), TensorConvention.Haeberlen)
            isotropy = tensor.isotropy
        except Exception as e:
            logger.warning(f"Entry {i}: failed to calculate isotropy: {e}")
            return None
    ...
    def _populate_element_resolved_magnetic_shielding(self, simulation, logger):
        """
        Populate the element_resolved_magnetic_shielding section with a dictionary
        mapping element symbols (or symbol_label) to lists of isotropy values,
        calculated directly from the magnetic shielding tensor values.
        """
        if not simulation.outputs or not simulation.model_system:
            logger.info(
                "No outputs/model_system found for element-resolved magnetic shielding."
            )
            return

        outputs_ref = simulation.outputs[0]
        model_system_ref = simulation.model_system[0]
        ...
        ms_list = getattr(outputs_ref, 'magnetic_shieldings', [])

        element_isotropy_list = [
            self._process_magnetic_shielding_entry(
        ...
        simulation.element_resolved_nmr_search = element_section

    def _process_electric_field_gradient_entry(
        self, i, efg, particle_states_ref, logger, ElementVzzEntry
        ):
        entity_ref = getattr(efg, 'entity_ref', None)
        value = getattr(efg, 'value', None)
        if entity_ref is None or value is None:
            logger.warning(f"Skipping entry {i}: missing entity_ref or value.")
            return None
        atom = next((ps for ps in particle_states_ref if entity_ref is ps), None)
        if atom is None:
            logger.warning(
                f"Skipping entry {i}: could not match entity_ref to any particle_state."
            )
            return None
        chemical_symbol = getattr(atom, 'chemical_symbol', None)
        if chemical_symbol is None:
            ...
            return None
        try:
            tensor = NMRTensor(np.array(value))
            tensor.order = TensorConvention.NQR
            eigenvalues = tensor.eigenvalues
            Vzz = eigenvalues[2]
            logger.debug(f"EFG entry {i}: eigenvalues={eigenvalues}, Vzz={Vzz}")
        except Exception as e:
            logger.warning(f"Entry {i}: failed to calculate Vzz: {e}. value={value}")
            return None
    ...
    def _populate_element_resolved_electric_field_gradient(self, simulation, logger):
        """
        Populate the element_resolved_electric_field_gradient section with a dictionary
        mapping element symbols (or symbol_label) to lists of Vzz values,
        calculated directly from the electric field gradient tensor values.
        """
        if not simulation.outputs or not simulation.model_system:
            logger.info(
                "No outputs/model_system found for element-resolved "
        ...
        element_section = getattr(simulation, "element_resolved_nmr_search", None)
        if element_section is None:
            from nomad_oasis_schema_parser_plugin.schema_packages.schema_package import (
                ElementResolvedNMRSearch,
            )
            element_section = ElementResolvedNMRSearch()
        efg_section = ElementResolvedElectricFieldGradient()
        efg_list = getattr(outputs_ref, 'electric_field_gradients', [])
        logger.warning(f"Total EFG entries found: {len(efg_list)}")

        element_vzz_list = [
            self._process_electric_field_gradient_entry(
    ...
    def parse(
        ...
        self._populate_element_resolved_magnetic_shielding(simulation, logger)

        self._populate_element_resolved_electric_field_gradient(simulation, logger)
